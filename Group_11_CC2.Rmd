...
1. **Cheryl Cheng** — 20421521
    + [BIOL343_CC](https://github.com/cherylxcheng/BIOL343_CC)
1. **Sarah Akagi** - 
...
```{r import-clean}
A1_Data_Excel <- read.csv("datafolder/A1_Data_Excel.csv")

NoBlank <- A1_Data_Excel[1:46, 1:8]

cc1_data <- subset(
  NoBlank,
  select = -c(population_name, latitude, longitude, avg_aug_temp)
)
...
$$\text{ovary width }\times \text{ovary height} = \text{ovary area}$$
...
area_means <- aggregate(
  cc1_data$ovary_area,
  list(cc1_data$population_type),
  mean
)
names(area_means) <- c("Population Type", "Ovary Area")
```
...
library(ggplot2)
ggplot(data = cc1_data, aes(x = ovary_area)) +
  geom_histogram(binwidth = 0.8) +
  facet_wrap("population_type", ncol = 1) +
  theme_classic() +
...
![_Solanum pimpinellifolium_](images\Hydroponic_g13.jpg)

Source: [BIOL 343 CC2 OnQ Site](https://onq.queensu.ca/d2l/le/content/1208667/viewContent/7105008/View)
